...
> ⚠️ 信号灯的扫描任务通过 OpenClaw 定时任务（cron jobs）管理，不依赖系统 crontab。

**扫描周期**：每 10 分钟一次，交易时段运行

| 任务名称 | Cron 表达式 | 执行时间 |
|---------|------------|---------|
| 信号灯-上午扫描(10点) | `0 0,10,20,30,40,50 10 * * 1-5` | 10:00 / 10:10 / 10:20 / 10:30 / 10:40 / 10:50 |
| 信号灯-上午扫描(11点) | `0 0,10,20,30 11 * * 1-5` | 11:00 / 11:10 / 11:20 / 11:30 |

- **执行命令**：`python3 /root/.openclaw/workspace/astock-signal/scripts/run_watch.py`
- **推送目标**：飞书 → 吴海赟（ou_ee2947ff311d4978679c2a2d4433f62a）
- **非交易时段**：周末 + 法定节假日不触发（含五一、十一等）
- **状态查看**：OpenClaw 控制台 → 定时任务

> 📌 如果 OpenClaw 重启后定时任务消失，需要在 OpenClaw 控制台重新创建。
