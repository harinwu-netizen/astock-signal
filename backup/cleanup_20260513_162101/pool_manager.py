...
class NixFlowPoolRecord:
    ...
    字段说明：
    - is_held:    是否已持仓（已持仓的不被淘汰）
    - is_traded:  是否已触发过交易信号
    - last_ai_prob: 上次AI评估概率
    """
    code: str
    name: str
    scan_date: str           # 入池日期
    score: float = 0.0
    close: float = 0.0
    net_outflow_10d: float = 0.0   # 万元，负值=净流出
    price_range_10d: float = 0.0   # %
    bottom_rise_10d: float = 0.0   # %
    amplitude_10d: float = 0.0    # %
    ma20_trend: str = ""
    avg_amount_10d: float = 0.0    # 万元
    ai_prob: str = ""              # 高/中/低
    ai_logic: str = ""             # AI核心逻辑
    ai_summary: str = ""          # AI摘要
    signal_type: str = ""          # 信号类型
    is_held: bool = False          # 是否已持仓（持仓的不淘汰）
    is_traded: bool = False       # 是否已触发过交易
    in_pool_date: str = ""        # 入池日期（同scan_date）
    last_ai_date: str = ""        # 上次AI评估日期
    last_l1_date: str = ""         # 上次L1检查日期
    status: str = "观察中"         # 观察中/已持仓/已出池
    exit_reason: str = ""         # 出池原因
    ...
    def from_candidate(c, ai_result=None) -> "NixFlowPoolRecord":
        ...
        ai_logic = ""
        ai_summary = ""
        if ai_result and hasattr(ai_result, "概率"):
        ...
        return NixFlowPoolRecord(
            ...
            amplitude_10d=c.amplitude_10d,
            ma20_trend=c.ma20_trend,
            avg_amount_10d=c.avg_amount_10d,
            ai_prob=ai_prob,
            ai_logic=ai_logic,
...
class NixFlowPoolManager:
    ...
    def _check_l1_for_record(self, record: NixFlowPoolRecord) -> bool:
        ...
        l1 = {
            "price_range": record.price_range_10d,
            "bottom_rise": record.bottom_rise_10d,
            "amplitude": record.amplitude_10d,
            "ma20_trend": record.ma20_trend,
            "avg_amount": record.avg_amount_10d,
        }
        # 严格模式：重新验证L1条件（不更新数据，仅检查）
        if l1["price_range"] < cfg.price_max_loss:
            return False
        if l1["price_range"] > cfg.price_max_gain:
            return False
    ...
    def daily_pool_check(self) -> dict:
        ...
        for record in list(self._pool):
            ...
            try:
                ...
                if l1 is None or not scanner._passes_l1_filter(l1):
                    reason = "L1条件不再满足"
                    self._eliminate(record, reason)
                    eliminated.append(record.code)
                    continue
    ...
    def weekly_maintenance(self, send_notification: bool = True) -> dict:
        ...
        for record in list(self._pool):
            ...
            # AI复查（持仓不满的才查AI）
            if record.ai_prob in ("低",):
                self._eliminate(record, f"AI概率低({record.ai_prob})")
                eliminated.append(record.code)
                continue
        ...
        # Step 4: 入池（排除已在池中 + 持仓已达上限）
        slots = self.MAX_POOL_SIZE - len(self._pool)
        added = []
        if slots > 0:
            for i, c in enumerate(candidates):
                r = ai_results[i] if i < len(ai_results) else None
                if not (r and r.is_recommended and r.概率 in ("高", "中")):
                    continue
                ...
                added.append(c.code)
                logger.info(f"[NixFlowPoolManager] ✅ 入池: {c.code} {c.name} (评分:{c.score:.0f}, AI:{r.概率})")
                if len(added) >= slots:
                    break
        else:
        ...
        result = {
            ...
            "added": added,
            "final_pool": [(r.code, r.name, r.score, r.ai_prob, r.is_held, r.status) for r in self._pool],
    ...
    def _notify_weekly(self, result: dict):
        ...
        try:
            ...
            for code, name, score, prob, is_held, status in pool:
                held_tag = "【已持仓】" if is_held else ""
                emoji = {"高": "🟢", "中": "🟡", "低": "🔴"}.get(prob, "⚪")
                lines.append(f"  {emoji}{code} {name} {held_tag}评分{score:.0f}")
    ...
    def print_status(self):
        """打印当前池状态"""
        print(f"\n{'='*70}")
        print(f"【逆流股票池】共 {len(self._pool)} 只  （上限 {self.MAX_POOL_SIZE}只）")
        print(f"{'='*70}")
        print(f"{'代码':<8} {'名称':<8} {'评分':>5} {'AI概率':>5} {'持仓':>4} {'信号类型':<16} {'状态'}")
        print("-" * 70)
        for r in self._pool:
            held = "✅" if r.is_held else "  "
            prob = r.ai_prob or "—"
            print(
                f"{r.code:<8} {r.name:<8} {r.score:>5.0f} {prob:>5} {held}   "
                f"{r.signal_type:<16} {r.status}"
            )
        print(f"{'='*70}")
        if self._exit_records:
            print(f"\n最近出池 ({len(self._exit_records)} 只):")
            for r in self._exit_records[-5:]:
                print(f"  {r.code} {r.name} | 出池原因: {r.exit_reason} | 原AI: {r.ai_prob}")
        print()
