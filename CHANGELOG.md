...
## v6.15（2026-06-15 16:18）⭐ 止损逻辑统一重构 + max_hold_days 崩溃修复

**主题：消除止损决策重复 + 修复 14:55 起的进程崩溃**

### 🐛 Bug 修复

| Bug | 根因 | 修法 |
|---|---|---|
| 14:55 起每轮扫描都崩 | `Position` 类**没有** `max_hold_days` 字段,但 `watcher.py:368/410` 引用了它 | 抽 helper,按 `market_regime` 选 `cfg.{strong,consolidate}_max_hold_days` |
| `market_regime != "WEAK"` 永真 | 字符串 `"WEAK"` vs `MarketStatus` 枚举永不相等 → 永远进入 and 右侧求值 → 必崩 | 改用 `is not MarketStatus.WEAK`(Python enum 官方推荐) |

**重现条件**:
- `Position.market_regime` 是枚举(从 JSON `"弱势"` → `MarketStatus.WEAK`)
- `MarketStatus.WEAK != "WEAK"` 永远 True → and 右侧求值 `position.max_hold_days` → AttributeError
- **任何有持仓的扫描都会崩**(今天 002202 金风科技 13:15 建仓,导致 14:55 后全崩)

### 🔄 重构

**前**:`_handle_stop_loss_signals` 和 `_handle_stop_loss_for_all_positions` 各有 4 个 if/elif,逻辑完全重复(还各有 1 处 bug 复制 2 份)

**后**:抽 `_decide_sell_reason(position, sig, cfg) -> Optional[str]` helper,两处都调

**净变化**:`monitor/watcher.py` 从 671 行 → 635 行(代码反而变少)

### ✅ 验证

新增 `scripts/test_v615_sell_reason.py` 单元测试,**12/12 通过**:
- 4 个止损条件 × 4 种市况
- 边界: 29 天不触发 / 10 天触发
- 优先级: ATR > 持仓超期 > 亏损超限 > RSI 反弹
- 真实数据: 从 `{"market_regime": "弱势"}` 反序列化后,`is not MarketStatus.WEAK` 正确
- 002202 当前场景: hold_days=0, 弱市, 止损 20.36 > 当前价 20.99 → **不触发**,符合预期

### 📅 生效

- 2026-06-15 16:18 手动备份到 `backup/v6.15_2026-06-15_sell_reason_unify/`
- 6/16 (周二) 10:00 守护脚本启动 watch 进程时加载 v6.15
- **资金流 fallback 仍受妙查 403 影响,本次未修**

### ⚠️ 未做(海赟未确认,暂不动)

- push2delay 健康探测误判(下午观察,资金流仍断)
- 妙查 403 限流的根因(等明早配额刷新)
- 005 002202 持仓 13:15 报告是**重试补发**(主因是飞书消息体超限)



---

## v6.16（2026-06-16 16:30）⭐ 资金流全面简化 + 守护脚本尾盘边界修复

### 🚨 触发原因

6/16 全天资金流 fallback 全面失效(84% 失败率)。海赟要求"先全面分析资金流查询逻辑,简化机制"。

**根因排查过程**:
1. 起初误判为 v6.11 健康探测缓存锁死(11:02-14:46 一直报"端点不健康")
2. 进一步看日志发现 15:00:01 守护脚本杀掉 watch 进程 → **真正根因是守护脚本 15:00 整点边界 bug**
3. 15:00 之后没有任何扫描,15:30 自动恢复的说法错误

### 🐛 Bug 1: 守护脚本 15:00 边界写错(根因)

**文件**:`run_watch_daemon.sh`

**问题**:
```bash
# 旧代码 -lt "1500" 导致 15:00:00 整点被判为非交易时间
if [ "$now" -ge "1300" ] && [ "$now" -lt "1500" ]; then
    return 0
fi
```

**修复**:`-lt "1500"` → `-lt "1501"`,15:00 整点仍算交易时间,15:01 才停

**影响**:信号灯原本只跑到 14:55,损失了 15:00 整点的尾盘最后一次扫描

### 🐛 Bug 2: v6.11 资金流防御机制叠加锁死(次因)

**文件**:`data_provider/money_flow.py`

**问题**:v6.11 上线时加了 3 个"防御"机制,叠加导致资金流全面失效:
1. **5 分钟健康探测缓存** → 1 次失败锁 5 分钟(实际 4 小时)
2. **30 分钟妙查退避** → 触限后 30 分钟内彻底跳过
3. **3 个数据源 fallback** → 妙查 + EM(EM 4 月起就废) + push2delay

实际效果:今日 84% 资金流请求失败(v6.9 时期约 30%)

**修复**:砍成"极简双源"+ 保留 15 分钟缓存:
- 删 `_is_push2healthy()` 30 行探测代码
- 删 `_mc_blocked_until` 退避全局变量
- 删 `_get_money_flow_eastmoney()` 整个函数(EM 死代码)
- 删 `.env` 的 `FUND_FLOW_BACKEND` 开关
- push2delay 从 Step 0(主源) → 兜底(妙查 403 时自动补位)

### 🆕 改动详情

| 文件 | 改动 | 行数变化 |
|------|------|----------|
| `run_watch_daemon.sh` | 15:00 边界 `-lt "1500"` → `-lt "1501"` | 0 |
| `data_provider/money_flow.py` | 重构为极简双源 | 368 → 182 (-50%) |
| `.env` | 删 `FUND_FLOW_BACKEND` 开关 | 0 |

### ✅ 验证(16:30)

- 语法检查:✅
- 7 只股票资金流 7/7 成功(妙查 403 时自动 fallback push2delay 拿到 -42 ~ +13309万 真实数据)
- 缓存测试:同只股票 2 次调用,第二次 0ms 命中
- 下游 `signal_unified.py` / `selector_backtest.py` 导入正常

### 📅 生效

- 守护脚本:明早 6/17 10:00 守护脚本启动时自动生效
- 资金流逻辑:6/16 收盘后已生效,明早 10:00 第一次扫描即跑新逻辑

### ⚠️ 未做(明早观察后再决定)

- **P2 精简 MoneyFlowData 字段**(16→6 个,减少 60%):保守起见暂不动,等明早资金流 7/7 跑稳后评估
- **妙查配额紧张**:14 次扫描 × 7 只 = 98 次/天,妙查 ~60-100 次配额仍超,需后续讨论 (A) 接受 push2delay 兜底 (B) 砍扫描频率到 10 次/天
...
盘中实时扫描:
  1. 妙想数据(主源,实时,字段全)
  2. push2delay(兜底,15 分钟延迟,字段缺 ddx/main_in/out)
...
## v6.19（2026-06-18 16:00）⭐ RSI_6 弱市卖出阈值调整

### 🎯 触发

6/18 10:00 金风科技 (sz002202) 触发 v6.15 "弱市 RSI 反弹到位"卖出规则
- 持仓 3 天，买入 ¥20.99 → 卖出 ¥21.98（+4.56%）
- 实际 RSI_6=89.0，触发了 RSI_6>65 规则
- 但 RSI_14 显示仅 43.9（用户视角的低位），让用户误以为不应该触发

海赟指出分析误判（RSI_6 ≠ RSI_14），并决定按 A 方案优化：
**weak_rsi_sell_threshold: 65 → 75**

### 🐛 改动

| 文件 | 改动 |
|------|------|
| `config.py:70` | 默认值 65.0 → 75.0 |
| `config.py:192` | env 默认 "65.0" → "75.0" |
| `.env` | 新增 `WEAK_RSI_SELL_THRESHOLD=75.0` |
| `scripts/test_v615_sell_reason.py` | 同步测试 + 加 test_006b (RSI_6=70 不触发) |
| `monitor/watcher.py` | 无改动 (走 cfg) |

### 📊 阈值效果对比

| RSI_6 | 阈值 65 (旧) | 阈值 75 (新) |
|-------|--------------|--------------|
| 70 (6/18 真实场景差值) | 触发 | 不触发 ✅ |
| 75 (临界) | 触发 | 不触发 ✅ |
| 80 | 触发 | 触发 |
| 89 (6/18 实际值) | 触发 | 触发（仍生效）|

> 注: RSI_6=89 仍会触发（>75），但保留了更多缓冲空间，减少短线暴拉后的误判卖出。

### ✅ 验证

- test_v615_sell_reason.py 12/12 通过
- test_006b 新增测试通过: RSI_6=70 在新阈值 75 下不触发
- 临界值验证: RSI_6=75 → None, RSI_6=80 → 触发

### 📅 生效

- 明早 6/19 10:00 第一次扫描自动生效（dotenv 加载 .env）
- 涉及路径: monitor/watcher.py → Watcher._decide_sell_reason → 4) 弱市 RSI 反弹到位分支
